...
contract LIQDAOBondStakeDepository is Ownable {
    ...
    address public immutable LIQD; // intermediate reward token from treasury
    address public immutable sLIQD; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints LIQD when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sLIQD gons remaining to be paid
        uint liqdPayout; //liqd amount at the moment of bond
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 LIQD ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) LIQD
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( LIQD ).safeTransfer( DAO, fee ); 
        }
        ...
        IERC20( LIQD ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        IStaking( staking ).claim( address(this) );
        uint stakeGons=ISLIQD(sLIQD).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            liqdPayout: _bondInfo[ _depositor ].liqdPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISLIQD(sLIQD).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sLIQD ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( LIQD ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sLIQD balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISLIQD(sLIQD).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to LIQD supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( LIQD ).totalSupply();
    ...
    /**
     *  @notice calculate amount of LIQD available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISLIQD(sLIQD).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or LIQD) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != LIQD );
        require( _token != sLIQD );
